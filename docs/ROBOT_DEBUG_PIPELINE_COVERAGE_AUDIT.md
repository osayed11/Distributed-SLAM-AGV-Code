...
|---|---|---|
| G1: D455 IMU is separate from RGB-D | Covered by `1.1 d455_imu_hid` and `1.1 realsense_motion_stream_gate`. The enumeration gate accepts either D455 `hidraw` or Linux IIO `HID-SENSOR-*` motion devices. The motion-only pyrealsense probe is enabled by the dataset gate when IMU is required. | Keep deciding per dataset whether the publishable IMU is base `/imu` or D455 `/camera/imu`; the doctor can prove enumeration separately from usable motion streaming. |
...
| Gap | Current coverage after this pass | Remaining operational note |
|---|---|---|
| Gap A: DDS discovery fails as fleet size grows | Covered by `3.3 dds_discovery`. When expected robot namespaces are configured, `robot_doctor` runs `ros2 node list` and fails if any namespace is missing. For more than four robots, `3.3 dds_discovery_server` warns unless `ROS_DISCOVERY_SERVER` is configured. | For fleet runs, pass every expected namespace with `--expected-robot-namespace /agv100` etc. Use a Fast-DDS discovery server if Wi-Fi multicast is unreliable. |
